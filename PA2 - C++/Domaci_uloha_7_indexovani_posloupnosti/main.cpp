...
//Comparator is "volitelný"
template <typename T_Type, typename T_Comparator = less<typename T_Type::value_type>>
class CIndex
{
public:
    CIndex ( T_Type inputSequence, const T_Comparator & comparator = T_Comparator () ) : m_inputSequence(move(inputSequence)), m_Comparator(comparator)
    {
    }
    ~CIndex () = default;
    //Iteration is same for list, string etc. because sometimes it represents whole words sometimes only chars
    set<size_t> search ( const T_Type & searchingFor ) const
    {
        set<size_t> out;
        int index = 0;

        //Iteration over the sequence this object represents
        for (auto it_mySeq = m_inputSequence.begin(); it_mySeq != m_inputSequence.end(); ++it_mySeq)
        {
            bool wasFound = true;

            //I have to increment this iterator X times in the inner loop (moving the pointer inside window)
            auto it_mySeq_save = it_mySeq;

            //Iterate over the find sequence, every time I "move" the window
            for (auto it_key = searchingFor.begin(); it_key != searchingFor.end(); ++it_key)
            {
                //Using compare function, to check, if they are equal
                if(m_Comparator(*it_mySeq_save, *it_key) || m_Comparator(*it_key, *it_mySeq_save))
                {
                    wasFound = false;
                    break;
                }

                //If I'm at the end of my sequence and everything was alright
                if(it_mySeq_save == m_inputSequence.end() && it_key == searchingFor.end())
                {
                    wasFound = true;
                    break;
                }
            ...
            //If in inner loop wasn't mistake found (there is a match)
            if(wasFound)
            {
                out.insert(index);
            }
...
private:
    T_Type m_inputSequence;
    T_Comparator m_Comparator;
};
