    ...
    bool CompanyUnique(const company & C, int & addr_whereToInsert, int & taxID_whereToInsert) //const - mby?
    {
        ...
        for (unsigned int i = 0; i < addr_sizeOfDatabase; ++i)
        {
            ...
            if(!addr_insertingFound)
            {
                ...
                {
                    if (cmpr_addr < 0) //Pokud aktuální záznam addresy v databázi, je dřív abecedně
                    {
                ...
                else if (i == addr_sizeOfDatabase - 1)
                {
                    if (cmpr_name < 0)
                    {
                       // cout << "   Found position for inserting - last position -1" << endl;
                        addr_whereToInsert = -1; //Push back
                    }
                    else //Pokud je dříve námi vkládaný string, tak nahradím na pozici i, ten aktuální
                    {
                        addr_whereToInsert = i;
                        //cout << "   Found position for inserting - last position I" << endl;
                    }
                ...
                else if (cmpr_addr < 0 && strcasecmp(database.at(i+1)._name.c_str(), C._name.c_str()) > 0)
                {
                    addr_whereToInsert = i + 1;
...
int               main           ( void )
{
   ...
   /* assert ( b1 . newCompany ( "ACME", "Thakurova", "666/666" ) );
    ...
    assert ( ! b1 . invoice ( "666/666/666", 500 ) );
    assert ( ! b1 . invoice ( "ACME", "Kolejni", 500 ) );
    cout << "bruh" << endl;
